...
function OrderTracking() {
  ...
  const getTimelineFromOrder = (order: Order): TimelineItem[] => {
    ...
    const currentStatusIndex = statusOrder.findIndex(item => item.key === order.status);
    ...
    return statusOrder.map((item, index) => {
      ...
      // Find the timestamp for this status from statusHistory
      const statusHistoryItem = order.statusHistory.find(history => history.status === item.key);
      const time = statusHistoryItem ? 
